...
void DallasTemperature::setPullupPin(uint8_t _pullupPin) {
	...
	pullupPin.configPort(1ULL << _pullupPin);
...
// if new resolution is out of range, it is aconstrained.
void DallasTemperature::setResolution(uint8_t newResolution) {

	bitResolution = constrain<int>(newResolution, 9, 12);
	DeviceAddress deviceAddress;
...
bool DallasTemperature::setResolution(const uint8_t* deviceAddress,
  ...
  else
  {
   ...
   // handle the sensors with configuration register
    newResolution = constrain<int>(newResolution, 9, 12);
    uint8_t newValue = 0;
    ScratchPad scratchPad;
