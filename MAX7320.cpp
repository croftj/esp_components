...
uint8_t MAX7320::readPortData(uint8_t addr) 
{
   uint8_t rv;
   esp_err_t err;
   ...
   if (m_initialized)
   {
      if (xSemaphoreTake(m_ioMutex, 1000 / portTICK_RATE_MS))
      {
         ...
         i2c_master_write_byte(cmd, addr << 1| I2C_MASTER_READ, (i2c_ack_type_t)1);
         i2c_master_read_byte(cmd, &rv, (i2c_ack_type_t)1);
         if ((err = i2c_master_cmd_begin(m_port, cmd, 1000 / portTICK_RATE_MS)) != ESP_OK)
         {
            if (err == ESP_ERR_TIMEOUT)
               ESP_LOGW(DEVICE_NAME, "I2C Bus is busy");
            else
               ESP_LOGW(DEVICE_NAME, "I2C Read Failed");
         }
         i2c_cmd_link_delete(cmd);
         xSemaphoreGive(m_ioMutex);
      }
      else
         ESP_LOGW("I2C_Read", "I2C Semaphore taken");
   }
   else
      ESP_LOGW("I2C_Read", "I2C Bus not initialized");
   return(rv);
}
