...
public:
   ...
   */
   ...
   void pinMode(unsigned int pin, uint8_t mode, bool pull_up = false)
   {
      gpio_mode_t m = (mode == 0) ? GPIO_MODE_INPUT_OUTPUT : GPIO_MODE_INPUT_OUTPUT;
...
//      std::cout << "initDtaGpio(): Initializing input; pullUP = " << pullUp
//                << ", bmsk: " << std::hex << _dtaGpio.bmsk 
//                << std::dec << std::endl;
      pinMode(pin, 0, pullUp);
      setupDtaGpio();
   }
...
//      std::cout << "initPwrCtrlGpio(): Initializing output" << std::endl;
      pinMode(pin, 1);
